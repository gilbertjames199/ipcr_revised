      ...
      if (Array.isArray(this.data)) {
        this.data.forEach(function (item) {
          if (item.ipcr_type === 'Core Function') {
            var val = _this.AverageRating(item.month === "0" || item.month === null ? _this.QuantityRate(item.quantity_type, item.TotalQuantity, 1) : _this.QuantityRate(item.quantity_type, item.TotalQuantity, item.month), _this.QualityRate(item.quality_error, item.quality_average), item.TimeRating == "" ? 0 : item.TimeRating);
            // alert(val);
            num_of_data += 1;
            sum += parseFloat(val);
...
function render(_ctx, _cache, $props, $setup, $data, $options) {
  ...
  }, "Submit", 8 /* PROPS */, _hoisted_8), $props.status == 0 ? ((0,vue__WEBPACK_IMPORTED_MODULE_0__.openBlock)(), (0,vue__WEBPACK_IMPORTED_MODULE_0__.createElementBlock)("button", {
    key: 0,
    "class": "btn btn-primary btn-sm mL-2 text-white",
